...
def handle_select_person(operator: DBOperator, name_query=None):
    ...
    if not operator.is_connected():
        print("error: cannot perform query, not connected to database.", file=sys.stderr)
        return
    try:
...
def handle_insert_course(operator: DBOperator, code=None, title=None, credits_str=None, dept_id_str=None, description=None):
    ...
    if not operator.is_connected():
        print("error: cannot perform insert, not connected to database.", file=sys.stderr)
        return
    try:
        if code is None:
        ...
        try:
            ...
            print("error: invalid input for credits. must be a positive integer.", file=sys.stderr)
            return
        try:
            department_id = int(dept_id_str)
        except ValueError:
            print("error: invalid input for department id. must be an integer.", file=sys.stderr)
            return
        ...
        if inserted_id is not None:
            operator.commit()
            print(f"successfully added new course with course_id: {inserted_id}.")
        else:
            print("failed to add the new course. check previous errors. transaction rolled back.")

    except Exception as e:
        print(f"an error occurred during course insertion: {e}", file=sys.stderr)
        operator.rollback()
...
def handle_delete_enrollment(operator: DBOperator, student_id_str=None, course_id_str=None, section=None, semester=None, year_str=None):
    ...
    if not operator.is_connected():
        print("error: cannot perform delete, not connected to database.", file=sys.stderr)
        return
    try:
        ...
        try:
            ...
            print("error: student id, course id, and year must be integers.", file=sys.stderr)
            return
        if not section or not semester:
             print("error: section number and semester cannot be empty.", file=sys.stderr)
             return
        ...
        if len(sys.argv) > 1 and sys.argv[1] == 'auto':
            ...
            confirm = input(f"found enrollment record(s): {existing}\nconfirm deletion? (yes/no): ").lower()

        if confirm == 'yes':
            deleted = operator.delete(table='enrollment', conditions=conditions)
            if deleted:
                operator.commit()
                print("enrollment record deleted successfully.")
            else:
                print("failed to delete the enrollment record. check previous errors. transaction rolled back.")
        else:
            print("deletion cancelled.")

    except Exception as e:
        print(f"an error occurred during enrollment deletion: {e}", file=sys.stderr)
        operator.rollback()

def handle_sql_console(operator: DBOperator):
    """handles option 4: execute raw sql commands."""
    if not operator.is_connected():
    ...
    if operator.is_connected():
        ...
        if len(sys.argv) > 1 and sys.argv[1] == 'auto':
            with open('test_output.txt', 'w') as f:
                ...
                handle_insert_course(operator, code="CS102", title="Intro to CS", credits_str="3", dept_id_str="1", description="Basic CS course")
                handle_delete_enrollment(operator, student_id_str="11", course_id_str="6", section="A", semester="Spring", year_str="2024")
        ...
        print(f"fatal: could not establish initial connection to database '{db_file}'. exiting.", file=sys.stderr)
        sys.exit(1)